...
Plus précisément, \jlv{le but ici est de tester} \jls{nous voulons tester} la capacité de généralisation de ces algorithmes, \ie~leur aptitude à maintenir des performances de détection similaires sur plusieurs corpus de scènes présentant des conditions expérimentales différentes.
...
\jlv{Le protocole adopté dans ce travail} \jls{La méthode qui est la notre} consiste à utiliser le modèle de scènes sonores proposé (\cf~Section~\ref{sec:ch4_modelForm}) afin de générer de nouveaux corpus de scènes simulées, scènes dont nous contrôlons les structures internes, ainsi que la nature des samples utilisés. 

Il s'agit alors d'éprouver les algorithmes sur ces nouveaux corpus, et de comparer leurs performances avec celles obtenues sur le corpus d'évaluation d'origine du challenge DCASE 2013. Les différences nous permettent de conclure quant à la capacité de généralisation des algorithmes considérés.
...
\end{tabular}
\end{center}
\caption{Classes d'événements sonores utilisées \jlc{"utilisées se rapporte à classes ou à événements sonores?} dans le cadre du challenge DCASE 2013}
...
\item les \emph{backgrounds}: les enregistrements de fonds sonores, \ie~des scènes amorphes (textures, \cf~Section~\ref{sec:ch4_eventTextureAmorphe}) ne possédant pas d'événement saillant \jls{perso je fermerais ma parenthèse après événement saillant}, \jlv{qui rendent compte} :jls{rendant compte} de l’environnement acoustique naturel \jlc{exit: inhérent au} \jls{du} lieu d'occurrence des événements. 
\end{itemize}

Les sons isolés de la banque \emph{QMUL} sont extraits de scènes enregistrées à l'université \emph{Queen Mary University of London} (QMUL) dans le cadre de la préparation du challenge AED DCASE-2013 \jlc{cela n'a-t-il pas déjà été dit à la L61?}, mais n'ayant pas été utilisées lors de l'évaluation,~\ie ne faisant pas partie des corpus de \emph{test} (\emph{test-QMUL}) et de \emph{development}. Ces sons isolés profitent donc des mêmes conditions d’enregistrement que les scènes du corpus \emph{test-QMUL} \citep{Giannoulis:2013a}. Le nombre d'événements par classe varie de 3 à 23. Les enregistrements de \emph{backgrounds} ont été réalisés sur les mêmes environnements acoustiques que ceux utilisés pour le corpus \emph{test-QMUL}, avec là encore les mêmes conditions d'enregistrements.
...
Pour le processus de simulation \emph{instance}, l'objectif est de générer des scènes simulées qui ressemblent le plus possible aux scènes du corpus \emph{test-QMUL}. \jlv{Cette ressemblance se comprend suivant deux aspects} \jls{Cette ressemblance s'entend sous deux aspects}:

\begin{itemize}
\item \emph{la structure temporelle}: le positionnement temporel en terme d'\emph{onsets} des événements sonores;
\item \emph{les niveaux sonores des événements}: la puissance du ratio entre l'énergie de l'événement et celle du \emph{background}, notée EBR (\emph{event to Background power Ratios}). L'$EBR$ d'un événement de $N$ échantillons est obtenu \jlc{obtenu s'accorde avec EBR. EBR est-ce masculin ou féminin?} en calculant le ratio en décibels entre de la valeur efficace (niveau $RMS$, \cf~Section~\ref{sec:ch5_recordDataSet}) du signal (\cf~Equation~\ref{eq:ch7_eq2}) de l'événement ($E_{rms}$) et du \emph{background}  $B_{rms}$):
...
Les positions \emph{onsets} et les $EBRs$ ainsi recouvrés \jlc{es-tu sûr de vouloir employer ici le terme recouvrés?} sont utilisés \jlc{recouvrés/utilisés "és" si et seulement si EBR est masculin} pour simuler un nouveau corpus de scènes. Pour chaque scène simulée, à chaque \emph{onset} d'une annotation (couple scène-annotateur), nous plaçons un événement de la même classe, choisi aléatoirement parmi la banque de sons isolés (\emph{QMUL} ou \emph{IRCCYN}). \jlv{Afin de garantir que les durées des samples sélectionnés ne soient pas trop longues, par rapport à celles d'origines, ces derniers sont coupés si leur durée est supérieure à la durée de l'annotation d'au moins $0.5$ secondes} \jlc{c'est mal dit. Il faut reformuler. Mais je veux avancer. J'y reviendrai peut-être après}. Les niveaux des événements des scènes simulées sont fixés par rapport aux $EBRs$ calculés sur les scènes enregistrées. 
...
Nous considérons $\mathcal{R}_{sujet}$ les notes de réalisme par sujet, moyennée \jls{notes moyennées} en considérant séparément les scènes de \emph{test-QMUL} et celles de \emph{instance-IRCCYN}.

Les $\mathcal{R}_{sujet}$ les scènes enregistrées et celles simulées \jlc{il s'agit là d'une énumération? les mathcal..., les scènes E et les scènes S? il manque alors des virgules. Je te laisse voir} sont respectivement de $4.4$ et $3.3$ (\cf~Figure~\ref{fig:xpRealism}), et présentent une différence significative (t-test appariées: $p<0.01$). D'après les commentaires des sujets, il semble que les scènes enregistrées n'aient pas été perçues comme très réalistes à cause de leur caractère scripté, les sujets ayant reconnu le fait qu'il s'agit de scènes jouées. 
...
\gl{TODO: deux analyses séparées QMUL et IRCCYN, moyenne par réplication et par sample, et paired ttest}
...
\item \emph{MFCC}: une représentation basée sur des coefficients cepstraux \jlc{spectraux?} calculées \jlc{calculées se rapporte à quoi?} sur échelle de Mel (MFCCs: \emph{Mel-Frequency Cepstral Coefficients}). Nous invitons le lecteur à se référer à la section~\ref{sec:ch6_mfcc} pour une description des MFCCs;
...
\end{itemize}
\jls{plutôt que "nous invitions le lecteur à se référer...3 qui est une formule un peu maladroite et surtout lourdingue, pourquoi n'utilises-tu pas le "cf."?}
...
\item comparaison entre \emph{test-QMUL} et \emph{abstract-QMUL}: les performances comparables montrent que les algorithmes sont robustes à un changement de positions temporels \jlc{"temporels" se rapporte à changement ou à positions? dans tous les cas il est mal accordé} des samples, si les paramètres structuraux des scènes ($EBRs$ et espacement inter-\emph{onsets}) \jlc{"espacement inter machin" espacement au singulier ou au pluriel? si au pluriel, il faut corriger tous les "espacements" déjà évoqués} sont conservés.
\end{itemize}
...
Pour les deux algorithmes, la première étape du traitement consiste à extraire des descripteurs sur l'ensemble des trames du signal, et la deuxième à classifier les trames. \jls{Pour chacun des algorithmes, la première étape consiste à extraire des descripteurs sur l'ensemble des trames du signal, la seconde consiste à classifier ces trames}.
...
Une inspection des matrices de confusion inter-classes révèle que c'est, pour les deux systèmes, l'étape de classification qui serait responsable de la dégradation des performances. Le tableau~\ref{tab:fp} affiche, pour tous les systèmes, le plus grand nombre de faux positifs moyenné \jlc{"moyenné" se rapporte à quoi?} sur l'ensemble des scènes pour les trois corpus, ainsi que la classe correspondante. Pour NVM et NR, une classe en particulier (NVM: toux, NR: porte-claquer) semble être détectée de manière abusive, augmentant drastiquement le nombre de faux positifs, et diminuant \emph{de facto} les résultats.

Nous concluons que, pour ces deux systèmes, la diminution des performances n'est probablement pas un artefact dû au processus de simulation, mais plutôt a un phénomène de sur-apprentissage de l'étape de classification. Considérant que ces systèmes sont les seuls à faire usage d'une approche de classification discriminative (NR: SVMs; NVM: RFs), nous conjecturons que le cadre d'entraînement proposé par le challenge DCASE, et notamment le faible nombre de samples disponibles pour l'apprentissage (20 par classe) \gl{TODO: A vérifier}, n'est pas adapté pour ces deux algorithmes.
...
Quand on considère une tâche de classification, un problème de taille est de savoir si le système évalué est capable de généraliser ses \jlc{"ses" pour les siennes?} capacités de classification à des données non-observées, mais qui correspondent aux classes considérées dans les corpus de \emph{train} et de \emph{development}.
...
\item évaluer la robustesse des systèmes \jlc{exit: lorsque} devant traiter des niveaux de bruits de fond différents. Une nouvelle fois, le système SCS est le seul a présenter des performances stables pour les différents $EBR$ considérés, et ce, probablement en raison d'une étape efficace de pré-traitement du bruit .
\end{enumerate}
...
Les facteurs influant sur les performances tels que le niveau de bruit, le niveau de \jlc{exit: la} polyphonie, la diversité intra-classe (différence acoustique entre la formation et les données d'essais) peuvent ainsi être évalués de façon indépendante, sans avoir la charge 
...
Enfin, deux types de scènes sont considérés, \jlc{exit: nommément} les scènes polyphoniques, et es scènes non-polyphoniques. Par scène polyphonique, nous comprenons une scène où les événements de différentes classes peuvent se recouvrer. Pour une scène non-polyphonique, un seul événement peut être actif à un instant donné.
